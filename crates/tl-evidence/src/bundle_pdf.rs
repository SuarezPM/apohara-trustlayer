...
pub fn render_bundle_pdf(bundle: &EvidenceBundle) -> Result<Vec<u8>, PdfExportError> {
    let (doc, page1, layer1) = PdfDocument::new(
        ...
        Mm(210.0_f32), // A4 width
        Mm(297.0_f32), // A4 height
        "page-1",
    );
    ...
    y -= 12.0;
    draw_horizontal_line(&doc, &mut current_layer, 15.0_f32, y as f32, 180.0_f32);
    y -= 8.0;

    // ─── Bundle metadata ───────────────────────────────────────────────
    write_text(&current_layer, &bold, 12.0, 15.0, y, "Bundle Metadata");
    y -= 7.0;
    write_kv(
        &current_layer,
        &font,
    ...
    y -= 6.0;
    write_kv(&current_layer, &font, 15.0, y, "Org ID:", &bundle.org_id);
    y -= 6.0;
    write_kv(
        &current_layer,
        &font,
    ...
    // ─── Disclosures ───────────────────────────────────────────────────
    write_text(&current_layer, &bold, 12.0, 15.0, y, "Disclosures");
    y -= 7.0;
    if bundle.disclosures.is_empty() {
        write_text(&current_layer, &font, 10.0, 15.0, y, "(no disclosures)");
        y -= 6.0;
    } else {
        for (i, disc) in bundle.disclosures.iter().enumerate() {
            ...
            for disclaimer in &disc.v1_disclaimers {
                ...
                if y < 30.0 {
                    ...
                    let (new_page, new_layer) =
                        doc.add_page(Mm(210.0_f32), Mm(297.0_f32), "page-next");
            ...
            y -= 2.0;
            if y < 30.0 {
                let (new_page, new_layer) = doc.add_page(Mm(210.0_f32), Mm(297.0_f32), "page-next");
                current_layer = doc.get_page(new_page).get_layer(new_layer);
                y = 280.0;
            }
        }
    ...
    // ─── Signature ─────────────────────────────────────────────────────
    write_text(&current_layer, &bold, 12.0, 15.0, y, "Signature");
    y -= 7.0;
    if let Some(sig) = &bundle.signature {
        let truncated = truncate_cose(&sig.cose_sign1_b64, 80);
        write_kv(&current_layer, &mono, 15.0, y, "COSE_Sign1:", &truncated);
        y -= 6.0;
        write_kv(&current_layer, &mono, 15.0, y, "Row hash:", &sig.row_hash);
        y -= 6.0;
    } else {
        write_text(&current_layer, &font, 10.0, 15.0, y, "(no signature)");
        y -= 6.0;
    }
    if let Some(fp) = &bundle.issuer_pubkey_fingerprint {
        write_kv(&current_layer, &mono, 15.0, y, "Issuer fp:", fp);
        y -= 6.0;
    }
    y -= 4.0;
    draw_horizontal_line(&doc, &mut current_layer, 15.0_f32, y as f32, 180.0_f32);
    ...
    // ─── Timestamp ─────────────────────────────────────────────────────
    write_text(&current_layer, &bold, 12.0, 15.0, y, "Timestamp");
    y -= 7.0;
    ...
    write_kv(&current_layer, &font, 15.0, y, "Provider:", tsa_label);
    y -= 6.0;
    if let Some(url) = &bundle.tsa_url {
        write_kv(&current_layer, &font, 15.0, y, "URL:", url);
        y -= 6.0;
    }
    y -= 4.0;
    draw_horizontal_line(&doc, &mut current_layer, 15.0_f32, y as f32, 180.0_f32);
    ...
    // ─── Disclaimers ───────────────────────────────────────────────────
    write_text(&current_layer, &bold, 12.0, 15.0, y, "Disclaimers");
    y -= 7.0;
    for disclaimer in &bundle.disclaimers {
        write_wrapped_text(&current_layer, &font, 9.0, 15.0, y, disclaimer, 180.0);
        y -= 4.5;
        if y < 30.0 {
            let (new_page, new_layer) = doc.add_page(Mm(210.0_f32), Mm(297.0_f32), "page-next");
            current_layer = doc.get_page(new_page).get_layer(new_layer);
    ...
    write_wrapped_text(&current_layer, &font, 9.0, 15.0, y, instructions, 180.0);
    let _ = y - 12.0;
