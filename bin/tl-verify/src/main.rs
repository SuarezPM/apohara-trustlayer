//! tl-verify — offline evidence packet verifier.
//!
...
#[derive(Debug, Error)]
enum VerifyError {
    #[error("usage: tl-verify <path/to/packet.json>")]
    Usage,
    #[error("io: {0}")]
...
fn run() -> Result<ExitCode, VerifyError> {
    ...
    println!("tl-verify: {}", path.display());
    print_steps(&steps);
...
fn main() -> ExitCode {
    let args: Vec<String> = env::args().collect();
    // Special dev-only flag: `tl-verify --gen-sample <out>`
