...
- **Inspect the encapsulated data model (P4.6 dedicated)**: every field on `EvidencePacket`, `SignedPacket`, `Tenant`, `DualLlm` is private; the public API is the 8 `pub fn` getters (no field access). The wire format is the only way to inspect a packet: `serde_json::to_value(&sp.packet())` or `sp.packet().bbaaar_outcome()` (the latter returns `Outcome` **by value** since `Outcome: Copy`, so tests don't need `.clone()` for PartialEq comparisons). The dead `evidence_packet_v` field was removed (0 reads / 0 writes); the schema version is now `pub const SCHEMA_VERSION: u32 = 1`. This is the **last `pub` field** from the 8th-auditor F-series.
...
## Phase 0-5 + P4.6 EvidencePacket — Production-ready + Code Quality — COMPLETE (2026-06-28, 17 atomic commits)
...
### F-series (code quality) — `9 commits` (P4.6 dedicated pass added)
...
- **P4.6 dedicated** `refactor(themis-orchestrator): EvidencePacket pub→private + 8 pub getters`. The plan estimated 141 access sites — the real count was 37 (the "141" came from regex false positives matching `tenant_id` on `Faction`/`Config`/etc.). 8 fields → `pub(crate)` (only `new()` + `attach_peer_verdict` can write); the dead `evidence_packet_v` field (0 reads / 0 writes outside the constructor) was removed; schema version → `pub const SCHEMA_VERSION: u32 = 1`. **8 pub getters**: `packet_id()` (Uuid Copy), `tenant_id() -> &str`, `invoice_id() -> &str`, `agent_decisions() -> &[AgentDecision]`, `generated_at_ms()` (i64 Copy), **`bbaaar_outcome() -> Outcome` (by value, Copy — lets tests do `sp.packet().bbaaar_outcome() == Outcome::Approve` without `.clone()`)**, `framework_mappings() -> &FrameworkMappings`, `peer_verdicts() -> &[PeerVerdict]`. Plus a `with_generated_at(ts_ms)` builder. 22 field-access sites migrated `.packet().X` → `.packet().X()`. The `evidence_packet_v` removal + private fields + 8 getters **closes the final `pub` field from the 8th-auditor F-series**.
...
| **P4.6 EvidencePacket** | 22 field-access sites migrated; 8 pub getters + 1 builder; dead `evidence_packet_v` removed; 0 regressions |
